...
class EncoderRNN(nn.Module):
    """"""
    def __init__(self, n_chars, embedding_size, n_layers=1, bidirectional=False):
        super(EncoderRNN, self).__init__()
        self.char_set = n_chars
...
class DecoderRNN(nn.Module):
    def __init__(self, n_chars, embedding_size, n_layers=1, bidirectional=False):
        ...
        self.embedding = nn.Embedding(self.n_chars, embedding_size)
        # add dropout
        self.gru = nn.GRU(embedding_size, embedding_size, batch_first=True)
        self.output_embedding = nn.Linear(self.hidden_size, n_chars)
        self.output_softmax = nn.Softmax()

    def embed(self, input_char_indexes):
        """char index to embedding"""
        batch_size = input_char_indexes.size()[0]
        embeded = self.embedding(input_char_indexes).view(batch_size, -1, self.hidden_size)
        return embeded

    def forward(self, embeded, hidden):
        """batch index goes first. Input and output are both size <,,n_chars>"""
        batch_size = embeded.size()[0]
        output, hidden = self.gru(embeded, hidden)
        output_embeded = self.output_embedding(output.view(-1, self.hidden_size)).view(batch_size, -1, self.n_chars)
        return output_embeded, hidden

    def extract(self, output_embeded):
        """char embedding to class indexes"""
        b_size, seq_len, n_chars = output_embeded.size()
        output_softmax = self.output_softmax(output_embeded.view(-1, n_chars))
        # TODO: alternatively: output_chars = output_softmax.multinomial(1).view(batch_size, -1)
        _, output_char_indexes = output_softmax.topk(1, dim=1)  # .view(batch_size, -1)
        return output_char_indexes.view(b_size, seq_len)
...
class VanillaSequenceToSequence(nn.Module):
    def __init__(self, input_lang, output_lang, hidden_size, n_layers=1, bidirectional=False):
        ...
        self.encoder = EncoderRNN(input_lang.n_chars, hidden_size, n_layers, bidirectional)
        self.decoder = DecoderRNN(output_lang.n_chars, hidden_size, n_layers, bidirectional)
    ...
    def forward(self, input, hidden, target=None, teacher_r=0, max_output_length=None):
        ...
        embeded_outputs = []
        hidden = encoded
        output_char_batch = self.get_SOS_vec(batch_size)
        ...
        while len(slices) < (max_output_length or self.args.MAX_OUTPUT_LEN) \
                and len(eos_flags) != 0:
            # char_slice size(b_size, 1), is correct
            ...
            output_slice_forced = output_char_batch \
            ...
            output_char_batch = self.decoder.extract(output_embedded)

            # Now add the slices to the output stack. char_slice(b_size, 1) -> size(b_size)
            slices.append(output_char_batch.view(batch_size))
            embeded_outputs.append(output_embedded.view(batch_size, -1))

            for ind, s in enumerate(output_char_batch):
